    ...
    <div className="min-w-0 space-y-16">
      ...
      <section className="text-center">
        <div className="mx-auto mb-5 flex min-w-0 flex-wrap items-center justify-center gap-2">
          <h1 className="break-words text-4xl font-extrabold tracking-tight text-foreground sm:text-5xl lg:text-6xl">{SITE_CONFIG.gameName}</h1>
        ...
        {SITE_CONFIG.trailerYoutubeId && <div className="mx-auto mt-5 max-w-2xl">
          <TrailerButton videoId={SITE_CONFIG.trailerYoutubeId} />
        </div>}
        <p className="mx-auto mt-5 max-w-3xl text-base leading-relaxed text-muted-foreground">{SITE_CONFIG.description}</p>
      ...
      <section><h2 className="text-3xl font-bold tracking-tight text-foreground">{home.faq.title}</h2><p className="mt-2 text-muted-foreground">{home.faq.description}</p><Accordion type="multiple" defaultValue={home.faq.items.map((_, index) => `item-${index}`)} className="mt-6 rounded-2xl border border-border bg-card/70 px-5">{home.faq.items.map((item, index) => <AccordionItem key={item.question} value={`item-${index}`}><AccordionTrigger className="text-left text-foreground">{item.question}</AccordionTrigger><AccordionContent className="leading-7 text-muted-foreground">{item.answer}</AccordionContent></AccordionItem>)}</Accordion></section>

      {/* Final CTA (curated, stays in JSON) */}
      <section className="rounded-3xl border border-border bg-gradient-to-br from-muted to-card p-8 text-center"><h2 className="text-3xl font-bold tracking-tight text-foreground">{home.finalCta.title}</h2><p className="mx-auto mt-3 max-w-2xl text-muted-foreground">{home.finalCta.description}</p><div className="mt-6 flex flex-wrap justify-center gap-3"><Button asChild size="lg"><Link href={localizeHref(SITE_CONFIG.footerGuideLinks.beginnerGuide, locale)}>{home.finalCta.primary}<ArrowRight className="ml-2 h-4 w-4" /></Link></Button>{SITE_CONFIG.officialLinks.game && <Button asChild size="lg" variant="outline"><Link href={SITE_CONFIG.officialLinks.game}>{home.finalCta.secondary}</Link></Button>}</div></section>
    </div>
  );
}
